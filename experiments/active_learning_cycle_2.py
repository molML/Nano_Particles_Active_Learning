...
if __name__ == '__main__':
    ...
    best_hypers_uptake = optimize_hyperparameters(x=uptake_x, y=uptake_y, std=uptake_std,
                                                  ...
                                                  model='bnn',
                                                  method='grid_search')
    best_hypers_uptake = {'lr': 0.0001, 'hidden_size': 32, 'epochs': 10000, 'n_layers': 2}
    ...
    uptake_eval_results, uptake_rmse = evaluate_model(x=uptake_x, y=uptake_y, std=uptake_std, id=uptake_id,
                                                      filename=f'results/uptake_model_{CYCLE}_eval_bnn_{DATE}_.csv',
    ...
    torch.save(uptake_model, f'models/uptake_model_{CYCLE}_bnn_{DATE}_.pt')
    ...
    previous_picks = load_data(cycle=CYCLE, set='uptake', shuffle=True, omit_unstable=False)[3]
    ...
    screen_df = screen_predict(screen_x, screen_id, uptake_model, pdi_model, size_model,
                               f'results/screen_predictions_{CYCLE}_{DATE}_.csv')

    """ Sample acquisition ------------------------------------ """

    m = int(len(screen_df)*M_ACQUISITION_FRACTION)
    k = K_ACQUISITION_BATCHSIZE/2
    ...
    screen_df = screen_df.loc[screen_df['y_hat_pdi'] < 0.2]

    # Get some high confidence, high uptake formulations
    high_top_m = screen_df.nlargest(m, 'y_hat_uptake')
    # select top k most **certain** samples
    high_top_k = high_top_m.nsmallest(k, 'y_uncertainty_uptake')
    high = np.array(high_top_k['ID'])

    # Get some high confidence, low uptake formulations
    low_top_m = screen_df.nsmallest(m, 'y_hat_uptake')
    # select top k most **certain** samples
    low_top_k = low_top_m.nsmallest(k, 'y_uncertainty_uptake')
    low = np.array(low_top_k['ID'])

    picks = np.concatenate((low, high))
    ...
    picks_df.to_csv(f'results/low_high_validation_{CYCLE}_{DATE}.csv', index=False)
