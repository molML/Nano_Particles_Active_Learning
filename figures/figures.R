...
# colours = c('#3a5f75', '#7f9ed2', '#e87076')
...
pca_res_doe <- prcomp(library_doe[,10:14], scale. = TRUE)
...
p_a1 = ggplot(pca_df_doe, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  ...
  # scale_alpha_manual(values=(c(0.75, 0.025))) +
  # scale_color_manual(values=c('#3a5f75', '#efa00b','#b9b08d')) +
  scale_color_gradient(low = "lightgrey", high = "lightgrey", na.value = NA) +
  custom_theme + theme(legend.position = 'none')


p_a2 = ggplot(subset(df_gh, cycle_origin == 0), aes(x = cycle_type, y = Uptake, fill = as.character(cycle), color = as.character(cycle))) +
  labs(y = 'Measured uptake (fold)', x = '') +
  geom_boxplot(width=0.5, alpha=0.1, size=0.5, fatten=0.75) +
...
p_a = plot_grid(p_a1, p_a2, ncol=2, rel_widths = c(5,2))
...
# PCA
pca_res_explore <- prcomp(library_explore[,10:14], scale. = TRUE)
...
p_b1 = ggplot(pca_df_explore, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  ...
  custom_theme + theme(legend.position = c(0.1, 0.1), legend.direction="horizontal")

p_b2 = ggplot(subset(df_gh, cycle_origin == 1), aes(x = cycle_type, y = Uptake, fill = as.character(cycle), color = as.character(cycle))) +
  labs(y = 'Measured uptake (fold)', x = '') +
  geom_boxplot(width=0.5, alpha=0.1, size=0.5, fatten=0.75) +
  geom_point(alpha = 0.75, size=0.5) +
  scale_y_continuous(breaks = seq(0, 15, 5), limits = c(0,16), expand = expansion(mult = c(0.0, 0.0))) +
  scale_color_manual(values=colours[2]) +
  scale_fill_manual(values=colours[2]) +
  custom_theme


p_b = plot_grid(p_b1, p_b2, ncol=2, rel_widths = c(5,2))
...
# PCA
pca_res_exploit <- prcomp(library_exploit[,10:14], scale. = TRUE)
...
range(pca_df_explore$y_uncertainty_uptake)
...
p_c1 = ggplot(pca_df_exploit, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
...
p_c2 = ggplot(subset(df_gh, cycle_origin == 2), aes(x = cycle_type, y = Uptake, fill = as.character(cycle), color = as.character(cycle))) +
  labs(y = 'Measured uptake (fold)', x = '') +
  geom_boxplot(width=0.5, alpha=0.1, size=0.5, fatten=0.75) +
  geom_point(alpha = 0.75, size=0.5) +
  scale_y_continuous(breaks = seq(0, 15, 5), limits = c(0,16), expand = expansion(mult = c(0.0, 0.0))) +
  scale_color_manual(values=colours[3]) +
  scale_fill_manual(values=colours[3]) +
  custom_theme


p_c = plot_grid(p_c1, p_c2, ncol=2, rel_widths = c(5,2))

mean(subset(df_gh, cycle_origin == 0)$Uptake)
sd(subset(df_gh, cycle_origin == 0)$Uptake)
range(subset(df_gh, cycle_origin == 0)$PDI)

##### 4D Bars ####

p_d_bars = ggplot(df_gh, aes(x = ID2, y = Uptake, fill = cycle, color = as.character(cycle))) +
...
df_gh_long = melt(df_gh, id = c('ID2', 'ID', "cycle", "cycle_type", "cycle_origin" ,"pick_for_next_cycle"))

p4e_size = ggplot(subset(df_gh_long, variable %in% c("Z_ave")), aes(x = ID2, y=variable, color= as.character(cycle), size=value))+
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
...
p4e_pdi = ggplot(subset(df_gh_long, variable %in% c("PDI")), aes(x = ID2, y=variable, color= as.character(cycle), size=value))+
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
  geom_point() + custom_theme +
  theme(axis.text.x = element_blank(),
...
p4e_form = ggplot(df_gh_long1, aes(x = ID2, y=variable, color=cycle, size=value))+
  scale_color_manual(values=colours) +
  labs(x='Selected NPs over cycles', y='NP Component') +
  geom_point(size=df_gh_long1$value2) +
  custom_theme + 
  theme(axis.text.x = element_text(size=7, face="plain", colour = "#1e3648", angle = 90, hjust=1, vjust=0.5))


p_d = plot_grid(p_d_bars, p4e_form, ncol=1, labels = c('d', 'e'), label_size=8, 
          rel_heights = c(10, 10))


##### Fig 4 ####

fig4_upper = plot_grid(p_a, p_b, p_c, ncol=3, labels = c('a', 'b', 'c'), label_size=8)
fig_4 = plot_grid(fig4_upper, p_d, ncol=1, labels = c('', ''), label_size=8, rel_heights = c(2.5,3))
fig_4
...
p5_a = ggplot(pca_df_val, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  ...
  scale_color_gradient(low = "darkgrey", high = "white", na.value = NA) +
  custom_theme + theme(legend.position = c(0.2, 0.2), legend.direction="horizontal",
...
# # Change stdev into 90% CI
# validation_picks_pred$error_upper = validation_picks$Uptake_pred_95
# validation_picks_pred$error_lower = validation_picks$Uptake_pred_5
...
df5_c = rbind(validation_picks_true, validation_picks_pred)
df5_c[df5_c$cycle_type == 'high-uptake' & df5_c$type == 'measured',]$type = 'measured_high'
df5_c$type = factor(df5_c$type, levels = c('predicted', 'measured', 'measured_high'))

df5_c_l = subset(df5_c, cycle_type == 'low-uptake' & type == 'measured')
df5_c_h = subset(df5_c, cycle_type == 'high-uptake' & type == 'measured_high')

id_order = c(df5_c_l$ID2[order(df5_c_l$Uptake)], df5_c_h$ID2[order(df5_c_h$Uptake)])

df5_c$ID2 = factor(df5_c$ID2, levels=unique(id_order))

# df5b = rbind(subset(df5_c, cycle_type == 'low-uptake' & type == 'measured'),
#              subset(df5_c, cycle_type == 'high-uptake' & type == 'measured_high'))
# 
# df5b$cycle_type = gsub('low-uptake', 'low', df5b$cycle_type)
# df5b$cycle_type = gsub('high-uptake', 'high', df5b$cycle_type)
# df5b$cycle_type = factor(df5b$cycle_type, levels = c('low', 'high'))
# 
# p_5b = ggplot(df5b, aes(x = cycle_type, y = Uptake, fill = as.character(type), color = as.character(type))) +
#   labs(y = 'Measured uptake (fold)', x = '') +
#   geom_boxplot(width=0.5, alpha=0.1, size=0.5, fatten=0.75) +
#   geom_point(alpha = 0.75, size=0.5) +
#   # scale_y_continuous(breaks = seq(0, 15, 5), limits = c(0,16), expand = expansion(mult = c(0.0, 0.0))) +
#   coord_cartesian(ylim = c(0, 15), xlim=c(1, 2), expand = TRUE) +
#   scale_color_manual(values=c('#7F9ED2', '#305088')) +
#   scale_fill_manual(values=c('#7F9ED2', '#305088')) +
#   custom_theme

p5_c = ggplot(df5_c, aes(y=Uptake, x=ID2, group=type, fill=type))+
  geom_bar(stat="identity", color="black", position=position_dodge(), width = 0.75, size=0.25) +
  labs(y='Measured uptake (fold)', x='Selected NPs\n') +
  ...
  geom_errorbar(aes(ymin=error_lower, ymax=error_upper), width=0.4, position=position_dodge(0.75), size=0.3) +
  custom_theme + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
                       axis.title.x = element_blank())
...
p5_heat = ggplot(df5_c_long, aes(x = ID2, y=variable, color=cycle_type, size=value))+
  scale_color_manual(values=c('#7F9ED2', '#305088')) +
  labs(x='Selected NPs over cycles', y='NP Component') +
...
p5_rightside = plot_grid(p5_c, p5_heat, ncol=1, rel_heights = c(5, 4))
p5_leftside = plot_grid(p5_a, ggplot() + theme_void(), ncol=1, rel_heights = c(5, 2))

p_c = plot_grid(p5_leftside, p5_rightside, ncol=2, rel_widths = c(5, 5))
p_c
